...
object ValidationSpec extends SimpleIOSuite {
  ...
  test("should raise if string or binary values exceed 2^32 - 1 bytes") {
    validation1(
      MsgpackItem.Str(ByteVector.empty.padLeft(Math.pow(2, 32).toLong)) -> new ValidationErrorAt(
        0,
        "String exceeds (2^32)-1 bytes"),
      MsgpackItem.Bin(ByteVector.empty.padLeft(Math.pow(2, 32).toLong)) -> new ValidationErrorAt(
        0,
  ...
  test("should raise on unexpected end of input") {
    validation(
      ...
      List(MsgpackItem.Array(2), MsgpackItem.Array(1), MsgpackItem.True) -> new ValidationError(
        "Unexpected end of input (starting at 0)"),
      List(MsgpackItem.Array(1), MsgpackItem.Array(1)) -> new ValidationError(
        "Unexpected end of input (starting at 1)"),
      List(MsgpackItem.Array(0), MsgpackItem.Array(1)) -> new ValidationError(
        "Unexpected end of input (starting at 1)"),
      List(MsgpackItem.Map(1), MsgpackItem.True) -> new ValidationError("Unexpected end of input (starting at 0)"),
      List(MsgpackItem.Map(1), MsgpackItem.Map(1), MsgpackItem.True, MsgpackItem.True) -> new ValidationError(
        "Unexpected end of input (starting at 0)"),
      List(MsgpackItem.Map(2), MsgpackItem.True, MsgpackItem.Map(1)) -> new ValidationError(
        "Unexpected end of input (starting at 2)"),
      List(MsgpackItem.Map(2), MsgpackItem.True, MsgpackItem.Map(1)) -> new ValidationError(
        "Unexpected end of input (starting at 2)"),
  ...
  test("should raise if extension data exceeds 2^32 - 1 bytes") {
    validation1(
      MsgpackItem.Extension(0x54, ByteVector.empty.padLeft(Math.pow(2, 32).toLong)) -> new ValidationErrorAt(
        0,
        "Extension data exceeds (2^32)-1 bytes")
    )
  }

  test("should raise if nanoseconds fields exceed 999999999") {
    validation1(
      MsgpackItem.Timestamp64(0xee6b280000000000L) -> new ValidationErrorAt(
        0,
        "Timestamp64 nanoseconds cannot be larger than '999999999'"),
      MsgpackItem.Timestamp96(1000000000, 0) -> new ValidationErrorAt(
        0,
        "Timestamp96 nanoseconds cannot be larger than '999999999'")
    )
  }
}
