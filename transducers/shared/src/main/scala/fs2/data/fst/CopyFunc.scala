...
object CopyFunc {
  case class CopyArg[T, C]() extends CopyFunc[T, C]
  case class CopyConst[T, C](out: C) extends CopyFunc[T, C]

  implicit def show[T, C: Show]: Show[CopyFunc[T, C]] = Show.show {
  ...
  implicit def CopyFuncChar[X]: Func.Aux[CopyFunc[Char, List[Either[String, X]]], Char, List[Either[String, X]]] =
    new Func[CopyFunc[Char, List[Either[String, X]]]] {
      ...
      def domain(f: CopyFunc[Char, List[Either[String, X]]]): LazyList[Char] =
        LazyList.from(BoundedEnumerable[Char].membersAscending)
  ...
  implicit def CopyFuncEitherListFunc[A, X](implicit
    ...
    new Func[CopyFunc[A, List[Either[A, X]]]] {
      ...
      def domain(f: CopyFunc[A, List[Either[A, X]]]): LazyList[Dom] = LazyList.from(A.membersAscending)
    }
  ...
  implicit def CopyFuncIdentityFunc[A](implicit
    ...
    new Func[CopyFunc[A, List[Id[A]]]] {
      ...
      def domain(f: CopyFunc[A, List[Id[A]]]): LazyList[Dom] = LazyList.from(A.membersAscending)
    }
}
