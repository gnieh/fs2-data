...
trait LiteralCellDecoders {
  ...
  inline given derived[T](using m: Mirror.Of[T]): CellDecoder[T] = new CellDecoder[T] {
    override def apply(cell: String): DecoderResult[T] = {
      inline m match {
        ...
        case s: Mirror.SumOf[T] =>
          val labels = constValueTuple[s.MirroredElemLabels]
          val types = summonAll[Tuple.Map[s.MirroredElemTypes, Mirror.Of]]
          val zipped = labels.zip(types).toList.asInstanceOf[List[(String, T)]]
