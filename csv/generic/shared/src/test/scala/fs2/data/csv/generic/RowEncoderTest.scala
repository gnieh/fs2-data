...
object RowEncoderTest extends SimpleIOSuite {
  ...
  @CsvValue("boonamed") case class Test(@CsvName("renamed") i: Int, s: String = "DEFAULT", j: Int)
  ...
  inline def makeDebug = codeOf(summon[Mirror.Of[Test]])

  loggedTest("debug") { log =>
    log.info(Debug.debugSingle(deriveRowEncoder[Test])).as(expect(false))
  }
