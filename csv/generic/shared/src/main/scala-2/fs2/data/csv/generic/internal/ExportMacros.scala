...
private[generic] class ExportMacros(val c: blackbox.Context) {
  ...
  final def exportRowDecoder[A](implicit a: c.WeakTypeTag[A]): c.Expr[Exported[RowDecoder[A]]] = {
    c.typecheck(q"_root_.shapeless.lazily[_root_.fs2.data.csv.generic.internal.DerivedRowDecoder[$a]]", silent = true) match {
      case EmptyTree =>
  ...
  final def exportRowEncoder[A](implicit a: c.WeakTypeTag[A]): c.Expr[Exported[RowEncoder[A]]] = {
    c.typecheck(q"_root_.shapeless.lazily[_root_.fs2.data.csv.generic.internal.DerivedRowEncoder[$a]]", silent = true) match {
      case EmptyTree =>
        c.abort(c.enclosingPosition, s"Unable to infer value of type $a")
      case t =>
  ...
  final def exportCsvRowDecoder[A](implicit a: c.WeakTypeTag[A]): c.Expr[Exported[CsvRowDecoder[A, String]]] = {
    c.typecheck(q"_root_.shapeless.lazily[_root_.fs2.data.csv.generic.internal.DerivedCsvRowDecoder[$a]]", silent = true) match {
      case EmptyTree =>
        c.abort(c.enclosingPosition, s"Unable to infer value of type $a")
      case t =>
  ...
  final def exportCsvRowEncoder[A](implicit a: c.WeakTypeTag[A]): c.Expr[Exported[CsvRowEncoder[A, String]]] = {
    c.typecheck(q"_root_.shapeless.lazily[_root_.fs2.data.csv.generic.internal.DerivedCsvRowEncoder[$a]]", silent = true) match {
      case EmptyTree =>
        c.abort(c.enclosingPosition, s"Unable to infer value of type $a")
      case t =>
  ...
  final def exportCellDecoder[A](implicit a: c.WeakTypeTag[A]): c.Expr[Exported[CellDecoder[A]]] = {
    c.typecheck(q"_root_.shapeless.lazily[_root_.fs2.data.csv.generic.internal.DerivedCellDecoder[$a]]", silent = true) match {
      case EmptyTree =>
        c.abort(c.enclosingPosition, s"Unable to infer value of type $a")
      case t =>
  ...
  final def exportCellEncoder[A](implicit a: c.WeakTypeTag[A]): c.Expr[Exported[CellEncoder[A]]] = {
    c.typecheck(q"_root_.shapeless.lazily[_root_.fs2.data.csv.generic.internal.DerivedCellEncoder[$a]]", silent = true) match {
      case EmptyTree =>
        c.abort(c.enclosingPosition, s"Unable to infer value of type $a")
      case t =>
