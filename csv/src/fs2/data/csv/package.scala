...
package object csv {
  ...
  def fromBytes[F[_], Header: Eq](withHeaders: Boolean, separator: Char = ',')(
      implicit F: ApplicativeError[F, Throwable],
      H: ParseableHeader[Header]): Pipe[F, Byte, CsvRow[Header]] =
    _.through(text.utf8Decode)
      .through(fromString(withHeaders, separator))

  def fromString[F[_], Header: Eq](withHeaders: Boolean, separator: Char = ',')(
      implicit F: ApplicativeError[F, Throwable],
        ...
        s.pull.uncons1.flatMap {
          case Some((line, rest)) =>
            ...
            def field(idx: Int, acc: StringBuilder, cont: Boolean): (String, Int, Boolean) =
              if (idx >= line.length)
                ...
                if (c == separator)
                  ...
                  if (cont)
                    // this is the closing quote of a continuation line
                    (acc.result, idx + 1, true)
                  else
                    quoted(idx + 1, acc)
                else if (cont)
            ...
            field(0, builder, cont) match {
              case (field, idx, true) =>
                fields(idx, NonEmptyList.one(field))
              case (field, _, false) =>
